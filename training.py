...
def get_random_chunk(
    ...
    """
    Gets a random batch of input and target sequences from a memory-mapped
    text files

    Args:
        batch_size: The number of blocks processed in parallel
        block_size: The maximum amount of tokens in a sequence of tokens
        device: The compute device
        split: Either "train" or "val", the training split is used to  
        teach the model, while the validation split checks if the model is
        correct
        string_to_int: Dictionary that maps characters to integers 

    Returns: 
        input_batch: Tensor of shape (batch_size, block_size) 
        - what the model will see
        target_batch: Tensor of shape (batch_size, block_size) 
        - what the model will predict

    """

    #Change filenames depending on the way you name your files
    ...
    with open(filename, 'rb') as f:
        #Memory Map file insted of loading into memory because it t
...
def estimate_loss(
    ...
    """
    Estimate the average loss (how inaccurate the model is) in both 
    ...
    Args: 
        ...
        block_size: The maximum amount of tokens in a sequence of tokens
        device: The compute device
        eval_iters: The number of batches where the loss is checked
        model: The model architecture 
        string_to_int: Dictionary that maps characters to integers 
        
    Returns:
        avg_loss: The average loss of the training and validation splits
    """
    ...
    for split in ['train', 'val']:
        ...
        for i in range(eval_iters):
            input_seq, target_seq = get_random_chunk(batch_size, 
                                                     block_size, 
                                                     device, 
                                                     split, 
                                                     string_to_int)
...
def train_model(
    ...
    """
    Trains the language model using the AdamW alogrithm
    ...
    Args:
        ...
        device: The compute device
        eval_iters: The number of batches where the loss is checked
        learning_rate: The size of the steps the model takes towards a lower loss
        max_iter: The number of training iterations
        model: The model architecture
        string_to_int: Dictionary that maps characters to integers 
    ...
    try: 
        for iter in range(1, max_iter+1):
            inputs, targets = get_random_chunk(batch_size, 
                                               block_size, 
                                               device, 
                                               "train",
                                               string_to_int)
            ...
            if iter % eval_iters == 0:
                ...
                print(
                      ...
                      f"Eval Iteration Time: {elapsed_time:.3f}, "
                      f"Average Step Time: {avg_step_time:.3f}",
                      flush=True)
...
class GPTLanguageModel(nn.Module):
    ...
    def __init__(self, 
        ...
        self.blocks = nn.Sequential(*[Block(block_size, dropout, n_embd, n_head=n_head) 
...
def main():
    ...
    try: 
        ...
        model = GPTLanguageModel(block_size, 
                                    device,
                                    dropout,
                                    n_embd, 
                                    n_head,
